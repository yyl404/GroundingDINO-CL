...
from finetune.obb import probiou_pairwise
...
def _to_xywhr(boxes: Tensor) -> Tensor:
    """
    Normalize target OBB boxes to [N, 5] xywhr format.

    Accepts:
    - [N, 5]: already xywhr
    - [N, 8]: polygon corners (x1,y1,...,x4,y4), converted via minAreaRect
    """
    if boxes.numel() == 0:
    ...
    if boxes.shape[-1] == 5:
        return boxes
    if boxes.shape[-1] == 8:
        from finetune.datasets.yolo import xyxyxyxy2xywhr

        return xyxyxyxy2xywhr(boxes)
    raise ValueError(f"Unsupported OBB target shape {tuple(boxes.shape)}. Expected [...,5] or [...,8].")


def _collect_predictions_obb(
    model,
    images: Sequence[Tensor] | NestedTensor | Tensor,
    classes: Sequence[str],
    *,
    ...
    pred_cls_logits_batch = outputs["pred_class_logits"].detach().cpu()
    if pred_boxes_batch.shape[-1] != 5:
        raise ValueError(
            f"OBB evaluation expects pred_boxes with 5 channels (cx,cy,w,h,theta), got {pred_boxes_batch.shape[-1]}."
        )
    if pred_cls_logits_batch.dim() == 2:
        pred_cls_logits_batch = pred_cls_logits_batch.unsqueeze(0)

    batch_preds: List[List[OBBDetection]] = []
    for bidx in range(pred_boxes_batch.shape[0]):
        pred_boxes = pred_boxes_batch[bidx]
        pred_cls_logits = pred_cls_logits_batch[bidx]
        scores, cls_ids = pred_cls_logits.max(dim=1)
...
def evaluate_obb(
    ...
    *,
    ...
    pbar = tqdm(data_loader, desc=progress_desc, leave=False)
    for images, targets in pbar:
        preds_per_img = _collect_predictions_obb(
            model,
            images,
            classes,
            device=device,
        ...
        for local_img_id, target in enumerate(targets):
            gt_boxes = _to_xywhr(target["boxes"].detach().cpu())
            gt_labels = target["labels"].detach().cpu()
            preds = preds_per_img[local_img_id]
            if len(preds) == 0:
                for cls_id in range(num_classes):
            ...
            pred_boxes_all = torch.stack([det.box_xywhr for det in preds], dim=0)
            pred_scores_all = torch.tensor([det.score for det in preds], dtype=torch.float32)
            pred_cls_all = torch.tensor([det.cls_id for det in preds], dtype=torch.long)
            ...
            for cls_id in range(num_classes):
                ...
                if n_gt > 0:
                    iou_mat = probiou_pairwise(pred_boxes_cls, gt_boxes_cls)
                    matched_gt = torch.zeros(n_gt, dtype=torch.bool)
                    best_ious, best_gt_idx = iou_mat.max(dim=1)
                    for pi in range(pred_scores_cls.shape[0]):
                        gi = int(best_gt_idx[pi].item())
