...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Visualize predictions for the first N batches and save side-by-side (pred vs GT) images.",
    )
...
def visualize_batches(
    ...
    for batch_idx, (images, targets) in enumerate(data_loader):
        if batch_idx >= vis_batch:
            break
        if zero_shot:
            outputs = wrapper.forward_zeroshot(images.to(device), classes=classes, aggregation_method="max")
        else:
            outputs = wrapper(images.to(device), classes=classes, aggregation_method="max")
...
def main() -> None:
    ...
    if args.vis_batch > 0:
        visualize_batches(
            ...
            score_threshold=args.eval_pr_score_threshold,
